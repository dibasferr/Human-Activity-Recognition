...
def K_means(N, device):
    ...
    limite= int(len(resultos)/2)
    ax.scatter(resultos[0:limite],resultos[0:limite,1],resultos[0:limite,2],c= colors[0:limite])
...
def aplicar_pca(features):
    ...
    X_scaled = scaler.fit_transform(features[:,:-1])
...
if __name__=="__main__":
    ...
    vetor_features = featureExtraction(dados)
